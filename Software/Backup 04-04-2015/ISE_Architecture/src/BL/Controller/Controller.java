...
public class Controller {
	...
	private UCreateMapFrame createMapFrame;
	private UGameFrame gameFrame;
	 ...
	 *            le niveau actuel
	 */
	public void changeInnerToGameFrame(String mapName) {
		innerMenuFrame.FermerMondeFenetre();
		// currentLevel = level;
		gameFrame = new UGameFrame(mapName);
	}

	/**
	 * passe de la fenetre jeu vers monde
	 */
	public void changeGameToInnerFrame() {
	 ...
	 *            le niveau actuel
	 */
	public void changeLoginToCreateMapFrame() {
		loginMenuFrame.closeLoginFrame();
		createMapFrame = new UCreateMapFrame();
	}

	/**
	 * passe de la fenetre jeu vers monde
	 */
	public void changeCreateMapToLoginFrame() {
		createMapFrame.closeCreateMapFrame();
		loginMenuFrame = new ULoginFrame();
	}
	...
	public void loadGame() {
		...
		String password = "";
		try {
			nom = JOptionPane.showInputDialog(loginMenuFrame, "Username:",
	...
	public void saveMap(Map currentMap) {
		try {
			ArrayList<Map> listMaps = readMapsDatabaseFile();
			listMaps.add(currentMap);
			writeMapToDatabaseFile(listMaps);
		} catch (Exception e) {
		}
	}

	public boolean isMapNameTaken(String mapName) {
		ArrayList<Map> listMaps = readMapsDatabaseFile();
		boolean nameTaken = false;
		for (int i = 0; i < listMaps.size(); i++) {
			if (listMaps.get(i).getMapName().equals(mapName)) {
				nameTaken = true;
			}
		}
		return nameTaken;
	...
	public Map getMapFromName(String mapName) {
		ArrayList<Map> listMaps = readMapsDatabaseFile();
		Map myMap = new Map("", 15, null, 1, 1);
		for (int i = 0; i < listMaps.size(); i++) {
			if (listMaps.get(i).getMapName().equals(mapName)) {
				myMap = new Map(listMaps.get(i).getMapName(), listMaps.get(i)
						.getMapSize(), listMaps.get(i).getTilesOnPath(),
						listMaps.get(i).getSceneryIcon(), listMaps.get(i)
								.getPathBackground());
				return myMap;
			}
		}
	...
	private void writeMapToDatabaseFile(ArrayList<Map> listMaps) {
		try {
			FileOutputStream fos = new FileOutputStream("maps");
			ObjectOutputStream oos = new ObjectOutputStream(fos);
			oos.writeObject(listMaps);
			oos.flush();
			oos.close();
		} catch (java.io.IOException e) {
		}
	...
	public ArrayList<Map> readMapsDatabaseFile() {
		ArrayList<Map> listMaps = new ArrayList<Map>();
		try {
			FileInputStream fichier = new FileInputStream("maps");
			ObjectInputStream ois = new ObjectInputStream(fichier);
			listMaps = (ArrayList<Map>) ois.readObject();
			ois.close();
		} catch (java.io.IOException e) {
		} catch (ClassNotFoundException e) {
		}
